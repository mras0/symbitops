...
function do_movem(src, dst, size) {
    ...
    if (src.length === 2 && src[1] === '+') {
        ...
        dst.forEach(function (r) { state[r] = state.do_ea(size, src)(); });
    } else if (dst.length === 2 && dst[1] === '-') {
        state.log_instruction('MOVEM', sizestr, pretty_reglist(src) + ', ' + format_ea(dst));
        src.reverse().forEach(function (r) { state.do_ea(size, dst)(state[r]); });
    } else {
        throw new Error('Not implemented MOVEM size=' + size + ' ' + src + ', ' + dst);
    }
}

exports.MOVEM   = function(src, dst) { return do_movem(src, dst, 16); };
exports.MOVEM.W = exports.MOVEM;
exports.MOVEM.L = function(src, dst) { return do_movem(src, dst, 32); };
